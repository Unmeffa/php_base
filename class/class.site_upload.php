 ...
 * <pre>
 *  $handle = new site_upload($_FILES['image_field']);
 *  if ($handle->uploaded) {
 ...
 * <pre>
 *  $handle = new site_upload('/home/user/myfile.jpg');
 * </pre>
 *
 * <b>How to set the language?</b><br>
 * Instantiate the class with a second argument being the language code:
 * <pre>
 *  $handle = new site_upload($_FILES['image_field'], 'fr_FR');
 *  $handle = new site_upload('/home/user/myfile.jpg', 'fr_FR');
 * </pre>
 *
...
class site_upload {
     ...
     */
    function site_upload($file, $lang = 'en_GB') {
        ...
        if ($this->uploaded)
		{
            ...
            if ($this->file_src_mime == 'application/octet-stream' || $this->file_src_mime == 'application/octet-stream; charset=binary' || !$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime))
			{
                ...
                if ($this->processed && $image_src) {
                   ...
                   if ($gd_version >= 2 && (is_numeric($this->image_overlay_percent) && $this->image_overlay_percent > 0 && !empty($this->image_overlay_color))) {
                    ...
                    switch($this->image_convert) {
                        ...
                        case 'jpg':
                            if (!$return_mode) {
                                $result = @imagejpeg($image_dst, $this->file_dst_pathname,100);
                            } else {
                                ob_start();
                                $result = @imagejpeg($image_dst, '',100);
