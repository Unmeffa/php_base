...
btnAdd?.addEventListener("click", async (e) => {
  ...
  formData.set("type", btnAdd.getAttribute("data-type"));
  formData.set("parentId", btnAdd.getAttribute("data-parentid"));

  try {
    const request = await fetch(
      `ajax/zone/form.php?${formData.toString()}&noCache=${Date.now()}`,
      {
        method: "GET",
        headers: {
...
const addZoneForm = async (form) => {
  const formData = new FormData(form);
  try {
    const request = await fetch("ajax/zone/add.php", {
      method: "POST",
      body: formData,
    });
...
btnsActive.forEach((btn) => {
  btn.addEventListener("click", async (event) => {
    ...
    try {
      ...
      const request = await fetch(`ajax/zone/active.php`, {
        method: "POST",
        body: formData,
      });
      if (request.ok) {
...
async function handlePrio(id, newPrio) {
  try {
    ...
    const response = await fetch("ajax/zone/prio.php", {
      method: "POST",
      body: formData,
    });
...
async function handleDelete(id) {
  try {
    ...
    const response = await fetch("ajax/zone/delete.php", {
      method: "POST",
      body: formData,
    });
...
deleteButtons.forEach((btn) => {
  btn.addEventListener("click", (event) => {
    ...
    if (
      confirm(
        `Souhaitez vous supprimer la zone ${name} ? Cette opération est irréversible.`
      )
    ) {
      handleDelete(id);
    }
...
document.body.addEventListener("submit", (e) => {
  ...
  if (target.getAttribute("name") === "addZone") {
    e.preventDefault();
    addZoneForm(target);
  }
});
